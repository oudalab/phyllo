...
def add_to_database(verse_entries, db):
    ...
    curs.execute("DELETE FROM texts WHERE author='Hugo of St. Victor'")
...
def main():
    # collection name: Hugo of Saint Victor
    hugoURL = 'http://www.thelatinlibrary.com/hugo.html'
    siteURL = 'http://www.thelatinlibrary.com'
    hugoOpen = urllib.request.urlopen(hugoURL)
    soup = BeautifulSoup(hugoOpen, "html5lib")
    textsURL = []
    ...
    title_dict_hugo, date_dict_hugo = get_title_and_date(soup)
    ...
    for work in textsURL:
        ...
        for p in para:
            ...
            # some Hugo texts have italic chapters
            ...
            if (get_verses(p)):
                for i, t in enumerate(get_verses(p)):
                    ...
                    # add the entry
                    add_entry_to_list(verses, title_dict_hugo, book, author, date, chapter,
                                      verse, text, work)
