...
def add_to_database(verse_entries, db):
    ...
    curs.execute("DELETE FROM texts WHERE author='William of Tyre'")
...
def main():
    # collection name: William of Tyre
    tyreURL = 'http://www.thelatinlibrary.com/williamtyre.html'
    siteURL = 'http://www.thelatinlibrary.com'
    tyreOpen = urllib.request.urlopen(tyreURL)
    soup = BeautifulSoup(tyreOpen, "html5lib")
    textsURL = []
    ...
    title_dict_tyre, date_dict_tyre = get_title_and_date(soup)
        ...
        for p in para:
            ...
            if (get_verses(p)):
                for i, t in enumerate(get_verses(p)):
                    ...
                    # add the entry
                    add_entry_to_list(verses, title_dict_tyre, book, author, date, chapter,
                                      verse, text, work)
