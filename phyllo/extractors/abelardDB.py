...
def add_to_database(verse_entries, db):
    ...
    curs.execute("DELETE FROM texts WHERE author='Petrus Abaelardus'")
...
def get_name_and_author_of_book(soup, url):
    ...
    book = re.sub(r'^\s+|\s+$|\n', '', book)
    author = re.sub(r'^\s+|\s+$|\n', '', author)
    author = author.title() # Abelardus' name seems to default to uppercase.
    return [book, author]
...
def main():
    ...
    abeURL = 'http://www.thelatinlibrary.com/abelard.html'
    ...
    title_dict_abe, date_dict_abe = get_title_and_date(soup)
        ...
        for p in para:
            ...
            if (get_verses(p)):
                for i, t in enumerate(get_verses(p)):
                    ...
                    # add the entry
                    add_entry_to_list(verses, title_dict_abe, book, author, date, chapter,
                                      verse, text, work)
