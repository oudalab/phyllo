...
def add_to_database(verse_entries, db):
    ...
    curs = db.cursor()
    #Replace old entries with new ones to prevent duplications.
    curs.execute("DELETE FROM texts WHERE author='Apuleius'")
...
def get_verses(soup):
    ...
    verses = filter(lambda x: len(x) > 3, verses)
...
def get_name_and_author_of_book(soup, url):
    ...
    book = pagetitle
    author = "Apuleius"
    # author name is not in the source. <title> tag contains only book name.
...
def main():
    # collection name: Apuleius
    apulURL = 'http://www.thelatinlibrary.com/apuleius.html'
    siteURL = 'http://www.thelatinlibrary.com'
    apulOpen = urllib.request.urlopen(apulURL)
    soup = BeautifulSoup(apulOpen, "html5lib")
    textsURL = []
    ...
    while ("http://www.thelatinlibrary.com/index.html" in textsURL):
        ...
        # also remove URLs done by other files
        textsURL.remove("http://www.thelatinlibrary.com/apuleius/apuleius.dog1.shtml")
        textsURL.remove("http://www.thelatinlibrary.com/apuleius/apuleius.dog2.shtml")
        textsURL.remove("http://www.thelatinlibrary.com/apuleius/apuleius.apol.shtml")
        textsURL.remove("http://www.thelatinlibrary.com/apuleius/apuleius.florida.shtml")
        textsURL.remove("http://www.thelatinlibrary.com/apuleius/apuleius.mundo.shtml")
        textsURL.remove("http://www.thelatinlibrary.com/apuleius/apuleius.deosocratis.shtml")
    logger.info("\n".join(textsURL))

    # extract data
    # get titles and dates of collections
    title_dict_apul, date_dict_apul = get_title_and_date(soup)
        ...
        for p in para:
            ...
            if (get_verses(p)):
                for i, t in enumerate(get_verses(p)):
                    ...
                    # add the entry
                    add_entry_to_list(verses, title_dict_apul, book, author, date, chapter,
                                      verse, text, work)
