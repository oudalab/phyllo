# This code scrapes Statius' collection.
...
def add_to_database(verse_entries, db):
    ...
    curs = db.cursor()
    #Replaces all available entries of Statius with new entries.
    curs.execute("DELETE FROM texts WHERE author='Statius'")
    for i, v in enumerate(verse_entries):
        curs.execute("INSERT INTO texts VALUES (?,?,?,?,?,?,?, ?, ?, ?, ?)",
                     (None, v["title"], v["book"], "Latin", v["author"], v["date"],
                      v["chapter"], v["verse"], v["text"], v["link"], "poem"))
...
def main():
    ...
    statiusURL = 'http://www.thelatinlibrary.com/statius.html'
    siteURL = 'http://www.thelatinlibrary.com'
    statiusOpen = urllib.request.urlopen(statiusURL)
    soup = BeautifulSoup(statiusOpen, "html5lib")
    textsURL = []
    ...
    # extract data
    title_dict_stat, date_dict_stat = get_title_and_date(soup)
    verses = []
    for work in textsURL:
        ...
        #Default values; poems do not have chapters.
        chapter = "1"
        verse = 0
        text = ""
        ...
        for p in para:
            ...
            if (get_verses(p)):
                for i, t in enumerate(get_verses(p)):
                    ...
                    # add the entry
                    add_entry_to_list(verses, title_dict_stat, book, author,
