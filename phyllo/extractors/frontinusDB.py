...
def parsecase1(ptags, c, colltitle, title, author, date, URL):
    ...
    for p in ptags:
        ...
        if len(text) <= 0 or text.startswith('Frontinus\n'):
            continue
        # let's try to find a bold chapter first
        findchapter = p.find('b')
        if findchapter is not None:
            chapter = text
            if text == 'SIGNORUM CONSPECTUS:':
                continue
        ...
        if passage.startswith('Frontinus'):
            continue
...
# main code
def main():
    # The collection URL below. Collection: Frontinus
    collURL = 'http://thelatinlibrary.com/frontinus.html'
    ...
    with sqlite3.connect('texts.db') as db:
        c = db.cursor()
        c.execute("DELETE FROM texts WHERE author='Frontinus'")
        for url in textsURL:
